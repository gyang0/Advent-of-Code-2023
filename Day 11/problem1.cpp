...
ll getDist(pii p1, pii p2){
	...
	for(int i = min(p1.f, p2.f); i < max(p1.f, p2.f); i++){
		// Skip by 2 for an empty row (galaxy expansion)
		if(emptyRow[i]) rowDist += 2;
		else rowDist++;
	}

	ll colDist = 0;
	for(int i = min(p1.s, p2.s); i < max(p1.s, p2.s); i++){
		// Skip by 2 for an empty column (galaxy expansion)
		if(emptyCol[i]) colDist += 2;
		else colDist++;
	}
